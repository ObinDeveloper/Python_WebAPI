...
def fetch_location(query):
    return requests.get(API_ROOT + API_LOCATION
                        + query).json()


def fetch_weather(woeid):
    return requests.get(API_ROOT + API_WEATHER
                        + str(woeid)).json()
...
def display_weather(weather):
    ...
    for entry in weather['consolidated_weather']:
        ...
        print(f"{date}\t{state}\thigh {high:2.1f}°C\tlow"
              f" {low:2.1f}°C")
...
def weather_dialog():
    try:
        ...
        print("Couldn't connect to server! Is "
              "the network up? ")
